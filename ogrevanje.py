...
def prepisi_in_zagrozi(vhodna, izhodna):
    prebrane_vrstice = []
    with open(vhodna, "r", encoding = "utf8") as f:
        prebrane_vrstice = f.readlines()
    with open(izhodna, "w", encoding="utf8") as f:
        for vrstica in prebrane_vrstice:
            print(vrstica.upper().replace(".", "!"), file=f, end="")
# =====================================================================@020347=
# 2. podnaloga
...
#     ['nepoliticno.txt']
# =============================================================================
def groznje_z_dovolj_klicaji(sez_datotek):
    ok_datoteke = []
    for datoteka in sez_datotek:
        st_klicajev = 0
        with open(datoteka, "r", encoding="utf8",) as f:
            for vrstica in f:
                st_klicajev += vrstica.count("!")
            if st_klicajev >= 3:
                ok_datoteke.append(datoteka)
    return ok_datoteke
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMDM0NiwidXNlciI6NzMyNH0:1nec2e:SKN8GcclQeeB5tNS86u_HJwte1I'
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMDM0NywidXNlciI6NzMyNH0:1nec2e:haCvhFzrfQyJKwSgKXkH9K-QU4c'
        try:
            test_cases = [
