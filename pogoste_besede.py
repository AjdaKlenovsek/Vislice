...
def najpogostejse_besede(vhod, st_besed, izhod):
    ...
    with open(vhod, "r", encoding="utf-8") as f:
        for vrsta in f:
            besede_v_vrsti = vrsta.strip().split(" ")
            for beseda in besede_v_vrsti:
                if beseda not in besede:
                    besede[beseda] = 1
                else:
                    besede[beseda] += 1
    urejene = [(-stevec, beseda) for beseda, stevec in besede.items()]
    urejene.sort()
    with open(izhod, "w", encoding="utf-8") as f:
        for i in range(st_besed):
            stevec, beseda = urejene[i]
            print(f"{beseda}, {-stevec}", file=f)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0Ijo5ODkxLCJ1c2VyIjo3MzI0fQ:1nec2e:nb_0FXKV2tclxFQ7KHNd6caR62g'
        try:
            pass
        except:
