...
def koliko(niz, datoteka):
    ...
    imena = set()
    imena_sez = niz.split(",")
    for ime in imena_sez:
        if ime in imena:
            continue
        else:
            imena.add(ime)
            stevci.append((ime, kolikokrat_se_pojavi(niz, ime)))
    with open(datoteka, "w", encoding="utf8") as f:
        for ime, stevec in stevci:
            print(f"{ime} {stevec}", file=f)
# =====================================================================@001512=
# 3. podnaloga
...
def koliko_urejen(vhod, izhod):
    koliko_iz_datoteke(vhod, izhod)
    with open(izhod, "r", encoding="utf-8") as f:
        pari = []
        for vrsta in f:
            ime, stevec = vrsta.strip().split()
            stevec = int(stevec)
            pari.append((-stevec, ime))
    pari.sort() #key=lambda par: (-par[1], par[0]))
    with open(izhod, "w", encoding="utf-8") as f:
        for stevec, ime in pari:
            print(f"{ime} {-stevec}", file=f)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoxNTEwLCJ1c2VyIjo3MzI0fQ:1nec2e:AI2oilpcuS7ZvS16M-H0p1K-gjY'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoxNTExLCJ1c2VyIjo3MzI0fQ:1nec2e:I1lIV8ghujX6wM07yxwSZVFSljk'
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoxNTEyLCJ1c2VyIjo3MzI0fQ:1nec2e:hVFIcjpb3uF9L-AIwItEa6BukaI'
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoxNTEzLCJ1c2VyIjo3MzI0fQ:1nec2e:q1uonoPcOTQukkxmE2U3DNKY3iE'
        try:
            test_cases = [
